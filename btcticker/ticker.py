...
class Ticker:
    ...
    def drawTextMax(
        self, x, y, max_w, max_h, text, font_name, start_font_size=20, anchor="la"
    ):
        ...
        if show_clock:
            w, h = self.drawText(
                ...
                % (
                    ...
                    str(last_block_time.strftime("%H:%M")),
            ...
            w, h = self.drawText(
                ...
                y,
                '%d blk %.0f %% %d:%d'
                % (remaining_blocks, (retarget_mult * 100 - 100), t_min, t_sec),
                font,
            )
        return w, h
    ...
    def drawFeesMax(self, x, y, mempool, anchor="la"):
        ...
        bestFees = mempool["bestFees"]
        if self.config.main.show_best_fees:
            w, h, font_size = self.drawTextMax(
                x,
                y,
                self.width - x,
                self.height - y,
        ...
        if layout == "big_two_rows":
            ...
            if mode == "fiat":
                if not self.config.main.show_block_time:
                    ...
                    line_str[0] = '%d - %s - %d:%d min' % (
                        mempool["height"],
                        str(last_block_time.strftime("%d.%b %H:%M")),
                        int(last_block_sec_ago / 60),
                        last_block_sec_ago % 60,
                    )
                ...
                if not self.config.main.show_block_time:
                    line_str[0] = '%s - %d - %d:%d - %s' % (
                        symbolstring + pricenowstring.replace(",", ""),
                        remaining_blocks,
                        t_min,
                        t_sec,
                        str(time.strftime("%H:%M")),
            ...
            if mode == "fiat":
                ...
                line_str[0] = '/{}{:.0f}'.format(
                    symbolstring, current_price["sat_fiat"]
                )
            ...
            if mode == "newblock":
                ...
                if mode == "fiat":
                    ...
                    line_str[3] = '{:.0f} sat/{}'.format(
                        current_price["sat_fiat"],
                        symbolstring,
                    )
            ...
            if mode == "fiat":
                ...
                if self.config.main.fiat == "usd":
                    line_str[1] = '{:.0f} sat/{}'.format(
                        current_price["sat_fiat"],
                        symbolstring,
                    )
                else:
                    line_str[1] = '{:.0f} /{} - ${} - {:.0f} /$'.format(
                        current_price["sat_fiat"],
                        symbolstring,
                ...
                if self.config.main.fiat == "usd":
                    ...
                    line_str[1] = '{} - ${} - {:.0f} /$'.format(
                        symbolstring + pricenowstring.replace(",", ""),
                        format(int(current_price["usd"]), ""),
                        current_price["sat_usd"],
                    )
                line_str[2] = '/{}{:.0f}'.format(
                    symbolstring, current_price["sat_fiat"]
                )
            elif mode == "moscowtime":
                line_str[0] = str(mempool["height"])
            ...
            if mode == "fiat":
                ...
                line_str[2] = (
                    ...
                    + " - $"
                    + format(int(current_price["usd"]), "")
                    + ' - {:.0f} /{}'.format(current_price["sat_fiat"], symbolstring)
                )
                line_str[3] = (
                    "/$" + "  " + str(self.price.days_ago) + "d : " + pricechange
                )
            ...
            ohlc_image = makeCandle(
                self.price.ohlc, figsize=(w, h), dpi=dpi, x_axis=False
            )
            ...
            pos_y += ohlc_h
            if self.width > 450:
                w_low, h_low, fs_low = self.drawTextMax(
                    self.width - 1,
                    self.height - 1,
                    self.width,
                    self.height - pos_y,
                ...
                w, h_symbolstring = self.drawText(
                    5, self.height - h_low - 10, line_str[3], self.font_side
                )
                ...
                w, h, font_size = self.drawTextMax(
                    5,
                    self.height - h_low - h_symbolstring - 20,
                    self.width,
                    (self.height - 20) / 2,
                    line_str[2],
            ...
            pos_y += int(h * 0.85)
            w, h = self.drawNextDifficulty(
                5, pos_y, remaining_blocks, retarget_mult, meanTimeDiff, time
            )
            pos_y += h
            w, h = self.drawText(5, pos_y, line_str[1], self.font_side)
            pos_y += h
