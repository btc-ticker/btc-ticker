...
class CoinGecko:
    ...
    def getCurrentPrice(self, currency):
        return float(
            self.cg.get_coins_markets(currency, ids=self.whichcoin)[0]["current_price"]
        )

    def getExchangeUSDPrice(self, exchange):
        rawlivecoin = self.cg.get_exchanges_tickers_by_id(
            exchange, coin_ids=self.whichcoin, include_exchange_logo=False
        )
    ...
    def getHistoryPrice(self, currency):
        ...
        rawtimeseries = self.cg.get_coin_market_chart_by_id(
            self.whichcoin, currency, self.days_ago
        )
        logger.info(
            "Got price for the last " + str(self.days_ago) + " days from CoinGecko"
        )
        ...
        i = 0
        while i < length:
            timeseriesdate.append(
                datetime.utcfromtimestamp(timeseriesarray[i][0] / 1000)
            )
